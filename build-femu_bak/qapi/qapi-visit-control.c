...
bool visit_type_EventInfo_members(Visitor *v, EventInfo *obj, Error **errp)
{
    if (!visit_type_str(v, "name", &obj->name, errp)) {
        return false;
    }
...
bool visit_type_EventInfo(Visitor *v, const char *name,
                 EventInfo **obj, Error **errp)
{
    bool ok = false;

    if (!visit_start_struct(v, name, (void **)obj, sizeof(EventInfo), errp)) {
        return false;
    }
    if (!*obj) {
        /* incomplete */
    ...
    if (!visit_type_EventInfo_members(v, *obj, errp)) {
        goto out_obj;
    }
    ok = visit_check_struct(v, errp);
out_obj:
...
bool visit_type_EventInfoList(Visitor *v, const char *name,
                 EventInfoList **obj, Error **errp)
{
    bool ok = false;
    EventInfoList *tail;
    size_t size = sizeof(**obj);
    ...
    for (tail = *obj; tail;
        ...
        if (!visit_type_EventInfo(v, NULL, &tail->value, errp)) {
            goto out_obj;
        }
    }
